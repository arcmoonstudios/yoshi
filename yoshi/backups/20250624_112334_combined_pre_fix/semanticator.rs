...
impl SemanticDeriveFramework {
    ...
    fn analyze_struct(&self, item_struct: &ItemStruct) -> Result<DeriveAnalysis, SemanticError> {
        ...
        if let Some(ref ai_engine) = self.ai_engine {
            if let Ok(ai_confidence) = self.run_ai_analysis(&struct_code, ai_engine) {
                analysis.confidence = f64::midpoint(analysis.confidence, ai_confidence);
    ...
    fn analyze_field(&self, field: &Field) -> Result<FieldAnalysis, SemanticError> {
        let field_name = field
            .ident
            .as_ref().map_or_else(|| "unnamed".to_string(), yoshi_std::ToString::to_string);
    ...
    fn generate_struct_derive_suggestions(
        ...
        if matches!(
            ...
            "data" | "config" | "message"
        ) && !self.config.forbidden_derives.contains("Serialize") {
        ...
        if (analysis.semantic_role == "config" || analysis.semantic_role == "data") && self.config.derive_strategy == DeriveStrategy::Aggressive {
        ...
        for analysis in analyses {
            ...
            for item in &mut syntax_tree.items {
                ...
                if let Some(name) = item_name {
                    if name == analysis.type_name && self.apply_derives_to_item(item, analysis)? {
                        modified = true;
                    }
                }
            }
            ...
            if attr.path().is_ident("derive") {
                if let Meta::List(meta_list) = &attr.meta {
                    ...
                    for derive in derive_str.split(',') {
                        let cleaned = derive.trim().replace(['(', ')'], "");
                        if !cleaned.is_empty() {
