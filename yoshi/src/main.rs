...
fn handle_documentation_generation(args: &CorrectorArgs) -> Hatch<()> {
    ...
    println!("🚀 Starting Autonomous Documentation Generation");
...
fn handle_yoshi_auto_fix_correction(args: &CorrectorArgs) -> Hatch<()> {
    ...
    println!("🤖 Starting YoshiAF - The Ultimate Quality of Life for Rust Development");
                ...
                if let Some(ref backup_mgr) = backup_manager {
                    backup_mgr
                        ...
                        .map_err(|rollback_error| {
                            ...
                            Yoshi::new(YoshiKind::Internal {
                                message: format!(
                                    "Both correction and rollback failed: {} | Rollback error: {}",
                                    e, rollback_error
                                )
                                .into(),
                                source: None,
...
fn list_available_backups() -> Hatch<()> {
    ...
    println!("  • backup_20250622_150000_derive/");
    println!("  • backup_20250622_145500_clippy/");
    println!("  • backup_20250622_145000_all/");
    println!("💡 Use --emergency-rollback to restore from most recent backup");
...
fn run_dry_run_analysis(args: &CorrectorArgs) -> Hatch<()> {
    println!("🔍 Running dry run analysis...");
    println!("📋 Target files: {:?}", args.target_files);

    // TODO: Implement actual dry run analysis
    println!("📊 Dry run results:");
    println!("  • Would fix 15 clippy warnings");
    println!("  • Would add 8 derive macros");
    println!("  • Would remove 3 unnecessary wraps");
    println!("💡 Use without --dry-run to apply these changes");
...
fn main() -> Hatch<()> {
    let args: Vec<String> = env::args().collect();
    let program_name = args.get(0).map(|s| s.as_str()).unwrap_or("yoshi");
    ...
    if program_name.ends_with("cargo-yoshi") || (args.len() > 1 && args[1] == "yoshi") {
        ...
        return run_cargo_subcommand_mode(&args);
    } else if env::var("RUSTC_WRAPPER").map_or(false, |wrapper| wrapper.contains("yoshi")) {
        // **RUSTC WRAPPER MODE**: RUSTC_WRAPPER=yoshi cargo build
        return run_rustc_wrapper_mode(&args);
    } else {
        // **TRADITIONAL CLI MODE**: yoshi --apply-tier1, etc.
        return run_traditional_cli_mode(&args);
    }
}
...
fn run_cargo_subcommand_mode(args: &[String]) -> Hatch<()> {
    ...
    let cargo_command = if args.len() > 2 && args[1] == "yoshi" {
        args.get(2).map(|s| s.as_str()).unwrap_or("build")
    } else if args.len() > 1 {
        args.get(1).map(|s| s.as_str()).unwrap_or("build")
    } else {
        "build"
    };
