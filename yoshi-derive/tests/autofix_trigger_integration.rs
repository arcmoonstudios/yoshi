...
fn test_yoshi_af_with_complex_error_patterns() {
    ...
    pub fn complex_function() -> Result<Vec<String>, Box<dyn std::error::Error>> {
        ...
        let first_char = results[0].chars().next().unwrap();
        results.push(first_char.to_string());

        Ok(results)
    }

    // Call the function to ensure it's used and doesn't panic.
    // We expect an error because the file "important.txt" doesn't exist.
    let _ = complex_function();
...
fn test_yoshi_af_with_generic_functions() {
    ...
    #[yoshi_error]
    fn generic_function<T: std::fmt::Display>(
        value: T,
